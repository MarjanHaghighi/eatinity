...
| AWS-0136 | SNS topics | Topics use the AWS-managed SNS KMS key. A dedicated customer-managed key would add recurring cost and publisher key-policy maintenance for low-sensitivity notifications. | 2026-12-31 |
| AWS-0013 | CloudFront default-certificate fallback | The fallback is used only without a custom domain. The `eatinity.ca` ACM certificate path enforces `TLSv1.2_2021`. | 2026-12-31 |
| AWS-0011 / S3 public-access checks | Files under `eatinity-backend/*.tf` | These files are retained historical Terraform and are not the deployment root. The active modular CloudFront configuration supports an optional WAF ARN. | 2026-12-31 |
